"""
Evaluate .tif images for an accompanying .tfw file, the bit depth, projection, tfw suggested unit, and pixel size. Writes a report .csv with findings.
...
Imports UtilityClass.py, ImageClass.py, TaxMapVariables.py
...
Check that each .tif has a .tfw file and write result to dictionary with filename: False/True).
...
Author: CJS
Date: 20171108
Revisions: 20180118 Refactored code to use @property, reduce bloat, extract variables, increase pythonic syntax
"""
def main(InputFileDirectory, NewFileDirectoryForAllImages):
    # IMPORTS
    ...
    strInputFileDirectory = InputFileDirectory
    strNewFileDirectoryForAllImages = NewFileDirectoryForAllImages
    ...
    # FUNCTIONALITY
        # Step through the directory and all subdirectories.
        # Create list of Image Objects for all files.
        # Create lists of image objects by filename based on the file extension (tfw and tif).
        # Notify the script user of the file extensions they are asking the script to examine and get a decision on proceed/exit
            # Basically a check for non TIF and TFW files such as zip files.
    try:
